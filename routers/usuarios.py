...
from core.python.auth.deps import get_current_admin_user, get_current_active_user
...
@router.get("/activos", response_model=List[dict], dependencies=[Depends(get_current_active_user)])
async def list_usuarios_activos():
    """Obtiene la lista básica de usuarios activos (id, nombre_completo) para selects."""
    return await usuario_service.get_usuarios_basico()

@router.get("", response_model=List[UsuarioResponse], dependencies=[Depends(get_current_admin_user)])
async def list_usuarios():
    """Lista todos los usuarios (solo admin)."""
    return await usuario_service.get_all_usuarios()

@router.post("", response_model=UsuarioCreateResponse, dependencies=[Depends(get_current_admin_user)])
...
@router.put("/{id_usuario}", response_model=UsuarioResponse, dependencies=[Depends(get_current_admin_user)])
...
@router.delete("/{id_usuario}", dependencies=[Depends(get_current_admin_user)])
