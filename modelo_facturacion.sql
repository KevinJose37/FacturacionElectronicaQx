...
CREATE TABLE FACTURACION.CORREO_ENTRANTE (
    ...
    MOTIVO_DESCARTE VARCHAR(100) NULL
);
...
CREATE INDEX IX_EVENTO_INGESTA_PENDIENTES 
...
CREATE TABLE FACTURACION.FACTURA (
    ...
    FECHA_CREACION          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    FECHA_ACTUALIZACION     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    VERIFICACION_GRAFICA_ESTADO VARCHAR(20) DEFAULT NULL,
    MOTIVO_RECHAZO              TEXT DEFAULT NULL,
    VERIFICACION_GRAFICA_DETALLE JSONB DEFAULT NULL,
...
-- =========================
-- FACTURA_CONTROL (req_reporte)
-- =========================

CREATE TABLE FACTURACION.FACTURA_CONTROL (
    ID_CONTROL               BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ID_FACTURA               BIGINT NOT NULL UNIQUE REFERENCES FACTURACION.FACTURA(ID_FACTURA),
    FECHA_ADMISION_PROVEEDOR DATE NULL,
    MEDIO_RECEPCION          VARCHAR(50) NOT NULL DEFAULT 'CORREO',
    FECHA_ENTREGA_CONTABILIDAD DATE NULL,
    NOMBRE_RECIBE_CONTABILIDAD VARCHAR(255) NULL,
    NOMBRE_PROVEEDOR         VARCHAR(300) NULL,
    NIT_PROVEEDOR            VARCHAR(20) NULL,
    NUMERO_FACTURA           VARCHAR(50) NULL,
    FORMA_PAGO               VARCHAR(20) NULL,
    ACUSO_RECIBIDO           BOOLEAN NOT NULL DEFAULT FALSE,
    RECIBIDO_BIEN_SERVICIO  BOOLEAN NOT NULL DEFAULT FALSE,
    ACEPTACION_EXPRESA       BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE INDEX IX_FACTURA_CONTROL_FACTURA ON FACTURACION.FACTURA_CONTROL (ID_FACTURA);
...
CREATE INDEX IX_CORREO_IMAP_UID ON FACTURACION.CORREO_ENTRANTE (IMAP_UID);
...
-- =========================
-- HABEAS DATA & AUDITORÍA
-- =========================

CREATE TABLE IF NOT EXISTS FACTURACION.AUDITORIA_ACCESO (
    ID_AUDITORIA BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ID_USUARIO BIGINT NOT NULL REFERENCES FACTURACION.USUARIO(ID_USUARIO),
    TIPO_ACCESO VARCHAR(50) NOT NULL, -- EJ: EXPORT_EXCEL, VIEW_INVOICE, SUPRESION_DATOS
    TABLA_AFECTADA VARCHAR(50) NOT NULL,
    QUERY_PARAMS JSONB,
    FECHA TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS IX_AUDITORIA_FECHA ON FACTURACION.AUDITORIA_ACCESO (FECHA DESC);
CREATE INDEX IF NOT EXISTS IX_AUDITORIA_USUARIO ON FACTURACION.AUDITORIA_ACCESO (ID_USUARIO);